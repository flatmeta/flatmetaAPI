...
<div class="card mb-3" id="customersTable" data-list='{"valueNames":["id","first_name","last_name","email","country","status"],"page":10,"pagination":true}'>
    <div class="card-header">
      <div class="row flex-between-center">
        <div class="col-4 col-sm-auto d-flex align-items-center pe-0">
          <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0 me-3">{{ "Users" }}</h5>

          <a class="btn btn-falcon-default btn-sm" href="{{ route('CreateUser') }}">
        ...
        <div class="col-8 col-sm-auto text-end ps-2">
          <form action="{{ route('Users',$parameters) }}" method="get"> 
            {{ csrf_field() }}
              <div class="row no-gutters">
                ...
                <div class="col pe-0 ">
                  <select name="status" class="form-select form-select-sm audience-select-menu">
                    ...
                    <option {{ (!empty($request) && $request->status == '1') ? "selected" : "" ; }} value="1">Verified</option>
                    <option {{ (!empty($request) && $request->status == '2') ? "selected" : "" ; }} value="2">Unverified</option>
                  </select>
                </div>
                <div class="col-auto">
        ...
        <table class="table table-sm table-striped fs--1 mb-0 overflow-hidden">
          <thead class="bg-200 text-900">
            <tr>
              ...
              <th class="sort pe-1 align-middle white-space-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sorting' => 'fullname','type' => $type]) }}'">FullName</th>
              <th class="sort pe-1 align-middle white-space-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sorting' => 'username','type' => $type]) }}'">UserName</th>
              <th class="sort pe-1 align-middle white-space-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sorting' => 'email','type' => $type]) }}'">Email</th>
              <th class="sort pe-1 align-middle white-space-nowrap" onclick="window.location.href='{{ request()->fullUrlWithQuery(['sorting' => 'status','type' => $type]) }}'">Status</th>
                ...
                <div class="dropdown font-sans-serif position-static">
                  ...
                  <div class="dropdown-menu dropdown-menu-end border py-0" aria-labelledby="customer-dropdown-0">
                    <div class="bg-white py-2">
                      <a class="dropdown-item text-info" href="{{ route('CreateUser',$user->id) }}">Edit</a>
                      <a class="dropdown-item text-danger" href="{{ route('DeleteUser',$user->id) }}" onClick="return confirm('Are you sure you want to delete?')">Delete</a>
                    </div>
                  </div>
                </div>
              </td>
...
@section('script')
    <script>
      var options = {
        valueNames: ["id","first_name","last_name","email","country","status","gdpr"]
      };
